# 🧭 Ruta de Aprendizaje: De Explorador a Estratega

!!! abstract "Objetivo de la Misión"
    Dominar el ciclo de vida cognitivo soberano. Avanza niveles resolviendo **Retos de Aprendizaje** (`learning-task`) y desbloquea nuevas capacidades para el ecosistema.

---

<div class="tactical-container" style="margin: 4rem 0; padding: 3rem;">
  <h3 style="margin-top: 0; text-align: center; color: var(--atlantyqa-navy); margin-bottom: 2rem;">🗺️ Mapa de Capacidades</h3>
  
```mermaid
graph TD
    L1(🟢 Explorador Cognitivo) --> L2(🔵 Constructor de Sentido)
    L2 --> L3(🟣 Ingeniero Cognitivo)
    L3 --> L4(⚫ Cooperador Estratégico)

    style L1 fill:#e7ae4c,stroke:#333,stroke-width:2px,color:#fff
    style L2 fill:#37a880,stroke:#333,stroke-width:2px,color:#fff
    style L3 fill:#e0e7ff,stroke:#333,stroke-width:2px,color:#182232
    style L4 fill:#f1f5f9,stroke:#182232,stroke-width:2px,stroke-dasharray: 5 5,color:#182232
```
</div>

## Niveles de Maestría

=== "🟢 Nivel 1: Explorador"
    <div class="feature-card" style="border-left: 5px solid #e7ae4c;">
        <h3>Explorador Cognitivo</h3>
        <p><strong>Enfoque:</strong> Entender el terreno y las herramientas básicas.</p>
        <ul>
            <li>✅ Instalar el entorno (`cogctl init`).</li>
            <li>✅ Ejecutar tu primer análisis de documento.</li>
            <li>✅ Entender la estructura de salida JSON.</li>
        </ul>
        <p><em>Recompensa: Insignia de "Analista Junior"</em></p>
    </div>

=== "🔵 Nivel 2: Constructor"
    <div class="feature-card" style="border-left: 5px solid #37a880;">
        <h3>Constructor de Sentido</h3>
        <p><strong>Enfoque:</strong> Mejorar la ingesta y visualización.</p>
        <ul>
            <li>✅ Personalizar el pipeline de limpieza de datos.</li>
            <li>✅ Crear visualizaciones simples en Streamlit.</li>
            <li>✅ Reportar un bug con trazabilidad completa.</li>
        </ul>
    </div>

=== "🟣 Nivel 3: Ingeniero"
    <div class="feature-card" style="border-left: 5px solid #182232;">
        <h3>Ingeniero Cognitivo</h3>
        <p><strong>Enfoque:</strong> Automatización y Orquestación (GitOps).</p>
        <ul>
            <li>✅ Automatizar despliegues con CI/CD.</li>
            <li>✅ Integrar nuevos modelos de NLP (spaCy/Transformers).</li>
            <li>✅ Optimizar rendimiento de inferencia.</li>
        </ul>
    </div>

=== "⚫ Nivel 4: Estratega"
    <div class="feature-card" style="border-left: 5px solid #000;">
        <h3>Cooperador Estratégico</h3>
        <p><strong>Enfoque:</strong> Arquitectura y Gobernanza.</p>
        <ul>
            <li>✅ Proponer RFCs (Request for Comments) de arquitectura.</li>
            <li>✅ Definir políticas de gobernanza de datos.</li>
            <li>✅ Mentoring a Exploradores nuevos.</li>
        </ul>
    </div>

<br><br>

## 🛠️ Cómo Progresar

El aprendizaje es activo. No hay exámenes, solo misiones reales.

1.  **Busca una Misión**: Ve a la pestaña de [Issues](https://github.com/atlantyde-labs/cognitive-suite/issues) y filtra por la etiqueta `learning-task`.
2.  **Ejecuta y Documenta**: Resuelve el problema en tu entorno local.
3.  **Comparte**: Envía un Pull Request (PR) pequeño.

!!! tip "Tip Pro"
    Un PR perfecto no es solo código. Incluye:
    *   **Contexto**: ¿Por qué haces este cambio?
    *   **Evidencia**: Capturas de pantalla o logs del "antes" y "después".

---

<div class="hero-cta" style="justify-content: center; margin-top: 4rem;">
  <a href="../talent-challenge-labs/" class="btn-primary">Ver Labs de Desafío →</a>
  <a href="https://github.com/atlantyde-labs/cognitive-suite/issues/new/choose" class="btn-secondary" target="_blank">Abrir Nuevo Issue</a>
</div>
