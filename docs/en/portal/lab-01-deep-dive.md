...
- [x] Spanish AI model downloaded (`python -m spacy download es_core_news_md`).
...
Create a file at `data/input/my_lab.txt` with sensitive content (names, budgets, emails). Then, "introduce it" to the Suite:
```powershell
python cogctl.py ingest my_lab.txt
```
*Why? Because the system must centralize and normalize files before analyzing them.*

### 2. Secure Analysis (The Suite's Brain)
Run the pipeline by activating the **Redaction** layer:
```powershell
$env:COGNITIVE_REDACT="1"; python cogctl.py analyze
```
*This is where spaCy looks for entities, Transformers analyzes sentiment, and our rules block financial leaks.*
...
```
*If you see all green checks, you have correctly configured the AI and Privacy engine!*

---

## 🏆 Extra Challenges (For Advanced Talents)
If you want to prove you understand the system as well as its creator, try this:

1. **The Multi-Tag**: Write a text that forces the AI to apply 4 or more tags (e.g., that speaks of an idea, a legal risk, and a pending action).
2. **Extreme Sentiment**: Try writing a text that gets a sentiment `score` higher than 0.85. What keywords "excite" the AI most?
