...
CS_LOG_PREFIX="install-gitea-runner-systemd"
...
SERVICE_SOURCE=${SERVICE_SOURCE:-"${SCRIPT_DIR}/gitea-runner.service.example"}
WRAPPER_SOURCE=${WRAPPER_SOURCE:-"${SCRIPT_DIR}/gitea-runner-service.sh"}
...
if [[ ! -f "${SERVICE_SOURCE}" || ! -f "${WRAPPER_SOURCE}" ]]; then
  cs_die "Service or wrapper source not found"
fi
