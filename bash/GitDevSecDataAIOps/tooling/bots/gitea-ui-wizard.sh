...
JSONL_PATH=$(cs_ui_prompt "Ruta del JSONL semantico" "${SCRIPT_DIR}/gitea-bot-clickops.jsonl")
...
ORG_NAME="${ORG_NAME}" TARGET_REPO="${TARGET_REPO}" GITEA_URL="${GITEA_URL}" \
BOT_USER="${BOT_USER}" EVIDENCE_REPO="${EVIDENCE_REPO}" JSONL_PATH="${JSONL_PATH}" \
python3 - <<'PY'
import json
import os
...
evidence_repo = os.environ["EVIDENCE_REPO"]
jsonl_path = os.environ["JSONL_PATH"]

record = {
    "record_id": f"bot-gitea-ui-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
    "platform": "gitea",
    "category": "gitea-ui",
    "title": "Gitea UI Bot Setup",
    "status": "info",
    "repo": {
        "owner": org,
        "name": repo,
        "url": f"{gitea_url}/{org}/{repo}",
    },
    "actors": [bot_user],
    "steps": [
        {"step_id": "create-user", "title": "Create machine user", "ui_path": "Admin Panel > Users > Create User", "required": True},
        {"step_id": "token", "title": "Generate access token", "ui_path": "User Settings > Applications > Generate Token", "required": True},
        {"step_id": "team", "title": "Add bot to team", "ui_path": "Organization > Teams", "required": True},
        {"step_id": "branch-protection", "title": "Configure branch protection", "ui_path": "Repo Settings > Branch Protection", "required": True},
        {"step_id": "evidence-repo", "title": "Create evidence repo", "ui_path": "Create Repo (private)", "required": True},
    ],
    "expected": ["Bot user created", "Token stored securely", "Evidence repo private"],
    "checks": [{"check_id": "ui.checklist", "status": "info", "detail": "Generated by wizard"}],
    "evidence": [{"type": "note", "ref": "wizard-output", "notes": "Generated by gitea-ui-wizard"}],
